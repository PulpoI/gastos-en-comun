<?php

return [
  'register_user' => 'api/register_user.php',
  'get_user_by_email' => 'api/get_user_by_email.php',
  'add_user_to_group' => 'api/add_user_to_group.php',
  'add_unregistered_user' => 'api/add_unregistered_user.php',
  'login_user' => 'api/login_user.php',
  'logout_user' => 'api/logout_user.php',
  'get_users_by_creator_id' => 'api/get_users_by_creator_id.php',

  'verify_token' => 'api/verify_token.php',
  'create_token' => 'api/create_token.php',

  'create_group' => 'api/create_group.php',
  'verify_group_password' => 'api/verify_group_password.php',
  'get_group' => 'api/get_group.php',
  'get_group_users' => 'api/get_group_users.php',
  'get_group_expenses' => 'api/get_group_expenses.php',
  'get_groups_by_user_id' => 'api/get_groups_by_user_id.php',
  'get_history_expenses' => 'api/get_history_expenses.php',

  'check_user_in_group' => 'api/check_user_in_group.php',
  'delete_group' => 'api/delete_group.php',
  'delete_user_to_group' => 'api/delete_user_to_group.php',
  'generate_history_expenses' => 'api/generate_history_expenses.php',

  'add_common_expense' => 'api/add_common_expense.php',
  'update_common_expense' => 'api/update_common_expense.php',
  'delete_common_expense' => 'api/delete_common_expense.php',

  'calculate_user_balances' => 'api/calculate_user_balances.php',

  'add_unregistered_user_to_group' => 'api/add_unregistered_user_to_group.php',
];
